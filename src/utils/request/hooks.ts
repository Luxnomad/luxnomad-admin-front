...
import { SWRConfig, LuxnomadResponse, UseQueryParamsConfig } from '@@utils/request/types';

// eslint-disable-next-line
export const useSWRList = <D = any, Q extends Record<string, any> = Record<string, any>>(path: string, config?: SWRConfig<D, Q>) => {
  const prevResponse = useRef<LuxnomadResponse<D>>();

  const { query, ...swrConfig } = config ?? {};
  const response = useSWR<LuxnomadResponse<D>>(swrConfig?.skip ? null : `${path}?${qs.stringify(query)}`, swrConfig ?? {});
